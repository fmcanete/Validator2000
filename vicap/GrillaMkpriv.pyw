...
class MyForm(wx.Frame):
	...
	def __init__(self):
		...
		wx.Frame.__init__(self, parent=None, title="Lector MARCAS PRIVADAS - VALIDATOR",size=(900,300), style = wx.DEFAULT_FRAME_STYLE)
		panel = wx.Panel(self)
		self.SetIcon(wx.Icon("validator_icono.ico"))

		itera = 0
		#Se colocan los nombres de las columnas
		nombres =['NUMEST','TERMINAL','NUMTAR','TIPOMOV','FECHA','HORA','NCUPON','MONEDA'
		,'IMPORTE','CUOTAS','NROAUT','DIASDIFER','ESTRUBRO','ESTDENOM','CODOP','FECHADIFER','NUMLOTE'
		,'FECHACIERRE','CODPLAN','CODRESPU','DATADIC','MODOING','FILLER'] 
		rango = len(nombres)   #Se toma la dimensión de la lista anterior
		LecturaCamposBasicos = pd.read_csv('CSV_MKPRIV.CSV',sep=';', index_col=0)   #Se abre el CSV creado anteriormente
