USE [CapturaStratus]
GO
/****** Object:  StoredProcedure [dbo].[sp_insertarCamposBasicos]    Script Date: 21/10/2021 17:22:19 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

ALTER PROCEDURE [dbo].[sp_insertarCamposBasicos]
AS
...
--from dbo.mov2000_camposBasicos)                  
	...
	SELECT	SUBSTRING(Info,36,16),SUBSTRING(Info,52,10),SUBSTRING(Info,112,8),SUBSTRING(Info,128,2),
			...
			cast (SUBSTRING (Info,135,13) +"." + SUBSTRING(Info,148,2) as float), SUBSTRING(Info,154,2), cast (SUBSTRING (Info,157,12)+ "." + SUBSTRING(Info,169,2) as float),
