...
class MyForm(wx.Frame):
	...
	def __init__(self):
		...
		wx.Frame.__init__(self, parent=None, title="Lector M240 - VALIDATOR",size=(900,300), style = wx.DEFAULT_FRAME_STYLE)
		panel = wx.Panel(self)
		self.SetIcon(wx.Icon("validator_icono.ico"))

		itera = 0
		#Se colocan los nombres de las columnas
		nombres =['establecimiento','Tipo','MkOffLine','MkBanda','Tarjeta','Libre1', 'NCupon',
		'Dia','Mes','Anio','Libre2','Autoriz','Importe','Cuotas','TNABco','Ramo','MConver',
		'Te1','TelOrig','TelDes','TelTpo']

		rango = len(nombres)   #Se toma la dimensión de la lista anterior
		LecturaCamposBasicos = pd.read_csv('CSV_M240.CSV',sep=';', index_col=0)   #Se abre el CSV creado anteriormente
